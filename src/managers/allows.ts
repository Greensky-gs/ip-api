...
export class Allow {
    ...
    public allow(ip: string, userid: string) {
        ...
        allow.findOrCreate({
            ...
            if (!u) m.update({ allowed: true, allowedAt: allowDate }).catch(console.log)
        })
    }
    ...
    private clean() {
        const now = Date.now()
        const valids = Object.keys(this.cache).filter(k => now - this.cache[k].allowedAt >= 86400000).map((k) => [k, this.cache[k]])
